...
def simulate_system(robot, x0, q_des, q_desdot, q_desddot, dt, T, D, dof, predictor_func, model, randomize=False, deviation=0):
    t = np.arange(0, T, dt)
    ...
    for i in range(1, len(t)):
        ...
        if i > nD:
            if randomize:
                ...
                prediction, predictions_arr = predictor_func(dt, q_vals[i-1], controls[i-1:i-1+nD], model) 
            controls[i-1+nD] = robot.compute_control(prediction, q_des[i-1+nD], q_desdot[i-1+nD], q_desddot[i-1+nD])
            if predictions_arr is not None:
                predictors[i] = predictions_arr
        else:
            controls[i-1+nD] = robot.compute_control(q_vals[i-1], q_des[i-1+nD], q_desdot[i-1+nD], q_desddot[i-1+nD])
            predictors[i] = np.tile(q_vals[i-1], nD).reshape(nD, len(q_vals[i-1]))
        #controls[i-1+nD] = robot.compute_control(prediction+ np.random.uniform(-0.5, 0.5, 2*dof), q_des[i-1+nD], q_desdot[i-1+nD], q_desddot[i-1+nD])
...
def gen_dataset(num_data,robot, dt, T, dof, D, scaling, period, joint_lim_min, joint_lim_max, deviation, filename):
    t = np.arange(0, T, dt)
    nD = int(round(D/dt))
    ...
    while index+1 < num_data:
        ...
        states, controls, predictors = simulate_system(robot, init_cond, q_des, qd_des, qdd_des, dt, T, D, dof, robot.compute_predictors, None, randomize=True, deviation=deviation)
        for i in range(len(sample_locs)):
            ...
            outputs[index] = robot.compute_predictors(dt, myStates, myControls, None)[1]
            index += 1
            if index+1 >= num_data:
                break
    end_time = time.time()
